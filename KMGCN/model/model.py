...
class GCNPredictor(nn.Module):
    ...
    def forward(self, node_feature, pcorr, data_graph):
        bz = data_graph.shape[0]     # batch

        h1 = torch.einsum('ijk,ijp->ijp', pcorr, node_feature)      
        ...
        h1 =  torch.einsum('ijk,ijp->ijp', pcorr, x)             
...
class Individual(nn.Module):
    ...
    def forward(self, t, nodes, pcorr, pseudo):        
        x = self.extract(t)                
        m = F.softmax(x, dim=-1)                                                                            
        ...
        return self.predictor(nodes, pcorr, data_graph)
...
class KMGCN(nn.Module):
    def __init__(self, config, node_size, node_feature_size, timeseries_size, out_dim):
        ...
        self.model_I = Individual(config['model'], node_size, node_feature_size, timeseries_size)       
    ...
    def forward(self, inputs, nodes, pcorr, pseudo, ages, genders):
        feature_I = self.model_I(inputs, nodes, pcorr, pseudo)
        feature_P = self.model_P(nodes, pseudo, ages, genders)

        bz = nodes.shape[0]
        x = self.lam_I*feature_I + self.lam_P*feature_P           
